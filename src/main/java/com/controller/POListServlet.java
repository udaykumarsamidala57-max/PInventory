...
public class POListServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (approveId != null) {
            try (Connection con = DBUtil.getConnection();
                 PreparedStatement ps = con.prepareStatement("UPDATE po_master SET Approval=? WHERE po_number=?")) {
        ...
        try (Connection con = DBUtil.getConnection();
             ...
             ResultSet rs = stmt.executeQuery("SELECT * FROM po_master ORDER BY PO_date DESC")) {

            while (rs.next()) {
                PO po = new PO();
                int poId = rs.getInt("PO_id");          // ✅ fetch PO_id (INT)
                ...
                // ✅ Fetch related items (with received qty from GRN and balance qty from stock)
                ...
                try (PreparedStatement ps2 = con.prepareStatement(
                    ...
                    ps2.setInt(1, poId);   // ✅ use PO_id instead of poNumber
                    ...
                    try (ResultSet rs2 = ps2.executeQuery()) {
                        while (rs2.next()) {
                            ...
                            item.setIndentNo(rs2.getString("po_no")); // keep po_no as is
